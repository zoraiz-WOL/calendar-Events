...
class Seeder {
    ...
    func printEntitiesCount() {
        ...
        for (entity, fetchRequest) in entities {
            if let nsFetchRequest = fetchRequest as? NSFetchRequest<NSFetchRequestResult> {
                let count = (try? container.viewContext.count(for: nsFetchRequest)) ?? 0
                print("The number of \(entity) entities in the database is: \(count)")
            } else {
                // Handle the case where 'fetchRequest' couldn't be cast
            }
        }
